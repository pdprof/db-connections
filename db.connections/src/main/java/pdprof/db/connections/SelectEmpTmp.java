 ...
 */
@WebServlet("/selectemptmp")
public class SelectEmpTmp extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		try {
			...
			Thread.sleep(1000);
			System.out.println("SELECT : select * from TMPLOYEE table start");
			con.createStatement().executeQuery("select * from TMPLOYEE");
			System.out.println("SELECT : select * from TMPLOYEE table end");
			ut.commit();
			response.getWriter().append("SELECT : select * from EMPLOYEE and TMPLOYEE tables in transaction timeout with ").append(timeoutStr).append(".");
		} catch (Exception e) {
